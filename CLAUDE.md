...
- `src/observability/` - Observability module; exports: `logger`/`createLogger` (pino + pino-loki; injects OTel trace context into every log line; stdout in dev, Loki push when `LOKI_URL` set), `appMetrics`/`createMetricsRegistry` (prom-client Registry; metrics: `http_request_duration_seconds`, `http_requests_total`, `db_query_duration_seconds`, `db_query_errors_total`, `active_connections`), `attachPrismaObservability` (Prisma query hooks for slow-query warnings + duration metrics), `sdk` (OpenTelemetry SDK from `instrument.ts`); used by `src/app.ts` for `GET /metrics` Prometheus endpoint and middleware; used by `src/middleware/logging.ts` and `src/middleware/monitoring.ts`
<!-- END AUTO-MANAGED -->
