...
const AUDITED_ROLES = new Set(["ADMIN", "PRINCIPAL", "TEACHER", "STAFF"]);
const AUDITED_METHODS = new Set(["POST", "PUT", "PATCH", "DELETE"]);

// Routes that are too noisy or auth-related — skip them
const SKIP_PATTERNS = [
  /^\/auth\//,
  /^\/notifications\//,
  /^\/queue\//,
...
function deriveAction(method: string, path: string): { action: string; entity?: string } {
  // Normalise: "/students/abc-123" → ["students", "abc-123"]
  ...
  // Sub-resource: e.g. PUT /ebooks/:id/file → "ebooks.file.update"
...
function extractEntityId(path: string): string | undefined {
  const segments = path.replace(/^\//, "").split("/").filter(Boolean);
  // Second segment is typically the ID: /students/:id
...
export function auditMiddleware(req: Request, res: Response, next: NextFunction) {
  ...
  // Log after the response completes so we know the status
  res.on("finish", () => {
    if (res.statusCode >= 400) return; // don't log failed attempts
...
/** Strip passwords and large blobs from the logged body */
